  ...
  if(date("Y-m-d",$appdate1)>=$cur_date){
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      $check_query = mysqli_query($con,"select apptime from appointmenttb where doctor='$doctor' and appdate='$appdate' and apptime='$apptime'");
        ...
        // Allow booking even if the doctor already has an appointment at this slot
        // Proceed with the booking logic as if slot is available
        // (You may want to add logic to warn or handle double-booking elsewhere)
        // For now, just insert the appointment anyway
        $query=mysqli_query($con,"insert into appointmenttb(pid,fname,lname,gender,email,contact,doctor,docFees,appdate,apptime,userStatus,doctorStatus) values('$pid','$fname','$lname','$gender','$email','$contact','$doctor','$docFees','$appdate','$apptime',1,1)");
        if($query)
        {
          echo "<script>alert('Your appointment successfully booked');</script>";
          ...
          <div class="card">
            <div class="card-body">
              ...
              document.getElementById('doctor').onchange = function updateFees(e) {
                // Find the selected option element itself
                var selectedOption = this.options[this.selectedIndex];
                // Get the data-docfees attribute (fee) directly from the selected option
                var selection = selectedOption.getAttribute('data-docfees');
              ...
              document.getElementById('spec').onchange = function updateSpecs(e) {
                ...
                document.getElementById('doctor1').value = document.querySelector(`[value=${values}]`).getAttribute('data-value');
                  ...
                  <div class="col-md-8"><input type="date" class="form-control datepicker" name="appdate" id="appdate" required/></div><br><br>
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <?php 
                    ...
                    // Get patient id from session for prescription query
