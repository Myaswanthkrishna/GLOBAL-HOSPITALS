...
// --- DEBUGGING START (Connection Check) ---
if (!$con) {
    die("DEBUG: Database connection failed in func2.php: " . mysqli_connect_error());
} else {
    // echo "DEBUG: Database connection successful in func2.php.<br>"; // Optional: uncomment to confirm connection
}
// --- DEBUGGING END ---
...
if(isset($_POST['patsub1'])){
  ...
  $contact=$_POST['contact'];
    ...
    // Input Validation
    if (strlen($cpassword) > 30) { // Check if cpassword exceeds table column length
        echo "<script>alert('Error: Confirmation password is too long (maximum 30 characters).'); window.history.back();</script>";
        exit();
    }
    ...
    if($password==$cpassword){
        ...
        // Prepare the query - REMOVED cpassword from insert
        $query="insert into patreg(fname,lname,gender,email,contact,password) values ('$fname','$lname','$gender','$email','$contact','$hashed_password');";

        // --- DEBUGGING START (Query Check) ---
        // echo "<hr>DEBUGGING INFO (func2.php - Stage 1: Before Insert):<br>";
        // echo "Executing Query: " . htmlspecialchars($query) . "<br>";
        // die("--- Script stopped before executing insert query --- "); // REMOVED THIS
        // --- DEBUGGING END ---

        $result=mysqli_query($con,$query);

        // **** Check if insert was successful AND affected 1 row ****
        if($result && mysqli_affected_rows($con) == 1){
            // Get the newly inserted patient ID
            $pid = mysqli_insert_id($con);
            // RESTORED SESSION ASSIGNMENTS:
            $_SESSION['pid'] = $pid; // Set pid in session
            ...
            echo "<script>alert('Registration successful! You will now be redirected to the patient panel.'); window.location.href='admin-panel.php';</script>";
            exit(); // Stop script after redirect
        } else {
             // **** Display error if insert failed or didn't affect rows ****
        }
    }
    else{
        // Passwords didn't match
        echo "<script>alert('Passwords do not match. Please try again.'); window.history.back();</script>";
        // header("Location:error1.php"); // Using JS alert instead
        exit();
    }
}
